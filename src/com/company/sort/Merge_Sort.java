...
public class Merge_Sort {
    ...
    public static int[] mergeSort(int[] array) {
        ...
        int size = 1; //Размер разбитых массивов для слияния.
        while (size < array.length) {
            for (int i = 0; i < array.length; i += 2 * size) {//Цикл слияния
                merge(currentSrc, i, currentSrc, i + size, currentDest, i, size);//Вызов метода слияния
            }
    ...
    private static void merge(int[] src1, int src1Start, int[] src2, int src2Start, int[] dest, int destStart, int size) {//src1 - первый массив, src1Start - параметр с какого элемента начинать слияние. dest - массив куда будут записываться элементы со слиянием
        ...
        int iterationCount = src1End - src1Start + src2End - src2Start;//Сколько иттераций нужно пройти циклу
