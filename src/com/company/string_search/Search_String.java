package com.company.string_search;
...
public class Search_String {
    ...
    static ArrayList<Integer> searchNaive(String text, String sample) {//слабая производительность
    ...
    public static ArrayList<Integer> KMPSearch(String text, String sample) {//Поиск Кнута-Морриса-Пратта
        ArrayList<Integer> found = new ArrayList<>();

        int[] prefixFunc = prefixFunction(sample);

        int i = 0;//Внутри текста
        int j = 0;//Внутри образца

        while (i < text.length()) {
            if (sample.charAt(j) == text.charAt(i)) {//Если совпали то движемся вперед
                j++;
                i++;
            }
            if (j == sample.length()) {
                ...
                if (j != 0) {
                    j = prefixFunc[j - 1];
                } else {//Если не совпали продвигаемся вперед
                    i = i + 1;
                }
            }
        }
